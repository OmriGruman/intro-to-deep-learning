...
class TorchTrainer():
    ...
    def fit(self, dl_train: DataLoader, dl_val: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            val_losses, val_accuracy = self.test_epoch(dl_val, verbose=verbose)
            ...
            # Comparing losses 
            curr_val_loss = torch.mean(torch.tensor(val_losses)).item()
            
            if actual_num_epochs == 1:
                best_val_loss = curr_val_loss
            elif best_val_loss > curr_val_loss:
                epochs_without_improvement = 0
                best_val_loss = curr_val_loss
            elif epochs_without_improvement + 1 == early_stopping:
                break
            else:
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss.backward()
        self.optimizer.step()
        num_correct = torch.sum(torch.argmax(preds, dim=1) == y).item()
        loss = loss.item()
        # ========================
